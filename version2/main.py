...
def main():
    ...
    search_sims = 50    # simulations per move for mcts
    ...
    c_pucts = [0.5, 1, 2]      # tree search exploration parameter
    eval_games = 100
    eval_list = []
    for eta in etas:
        eta_list = []
        for lmbda in lmbdas:
            ...
            for c_puct in c_pucts:
                net = Network([9,10,10,10,10,10])     # sizes controls the number of neurons in each layer of the network
                game = Game()
    ...
    # eta = 0.0001
    # lmbda = 0.1
    # c_pucts = [1] #[1,2,3,4,5,6] # [0.8, 1, 1.2, 1.4, 1.6, 1.8, 2]
    # for c_puct in c_pucts:
    #     net = Network([9,10,10,10,10,10], load=True)     # sizes controls the number of neurons in each layer of the network
    #     game = Game()
    #     net = training.policy_iteration(game, net, num_iterations, num_episodes, search_sims, eta, lmbda, c_puct)
